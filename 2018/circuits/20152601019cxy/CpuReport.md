...
![图2](./Another.png)
第三个时钟周期进行了跳转指令，我设置了将其多跳转了4个单位，所有第四个时钟周期显示的地址变为了20。第四个时钟周期进行了SLT指令，比较rs和rt寄存器里面的值的大小，如果rt的大，Result的值就为1。第五个时钟周期则是读对应地址的半个字数据，我选择了读地址为4的内容，因为第16位为1，所有符号扩展。
