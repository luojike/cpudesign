...
uint32_t readWord(unsigned int address) {
	...
	return *((uint32_t*)&(M[address]));//强制类型转换 
}
...
//	writeWord(36, (0x400<<20) | (2<<15) | (ORI<<12) | (4<<7) | (ALUIMM));
...
//	writeWord(16, 0x00100073);//EBREAK,默认跳转到 pc为4的位置
}
...
int main(int argc, char const *argv[]) {
	...
	while(c != 'n') {
		...
		switch(opcode) {
			case LUI://用于构建32位常数，将imm放到rd的高20位，低12位填0 
				cout << "Do LUI" << endl;
				R[rd] = Imm31_12UtypeZeroFilled;
				break;
			case AUIPC://在LUI基础上加上PC 
				cout << "Do AUIPC" << endl;
				cout << "PC = " << PC << endl;
			...
			case JAL://无条件跳转 
				cout << "Do JAL" << endl;
				R[rd]=PC+4;//将跳转指令后面指令的地址加载到rd 
				NextPC = PC+ Imm20_1JtypeSignExtended;//将偏移量加到pc后形成跳转目标地址    
				break;
			case JALR://jump and link
				cout << "DO JALR" << endl;
				R[rd]=PC+4;
				NextPC=R[rs1]+Imm11_0ItypeSignExtended;
				cout<<R[rs1]<<' '<<rs1<<' '<<Imm11_0ItypeSignExtended<<endl;
				break;
			case BRANCH://条件跳转 
				switch(funct3) {
					case BEQ:
			...
			case LOAD:
				switch(funct3) {
					...
					case LH:
						...
						cout << "LH Address is: " << src1+Imm11_0ItypeSignExtended << endl;
					...
					case LW:
						...
						cout << "LW Address is: " << src1+Imm11_0ItypeSignExtended << endl;
						temp_LW=readWord(src1+Imm11_0ItypeSignExtended);
					...
					case LHU:
						cout << "Do LHU" << endl;
						R[rd] = readHalfWord(Imm11_0ItypeSignExtended + src1) & 0x0000ffff;
						break;
					default:
			...
			case STORE:
				switch(funct3) {
					...
					case SH:
						...
						cout << "SH Addr and Data are: " << x << ", " << j << endl;
						writeHalfWord(x,j);
						break;
					case SW:
						cout << "DO SW" << endl;
			...
			case ALUIMM:
				switch(funct3) {
					...
					case SLTI:
						cout << "Do SLTI" << endl;
						if((int)src1<Imm11_0ItypeSignExtended)
							R[rd] = 1;
						else
							R[rd] = 0;
						break;
