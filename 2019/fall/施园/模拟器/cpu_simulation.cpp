...
int allocMem(uint32_t s) {                      //在此处定义存储器的功能 
	M = new char[s];
	MSize = s;
	return s;
}
...
char readByte(unsigned int address) {
	if(address >= MSize) {
		cout << "ERROR:  the address out of range in readByte" << endl;
		return 0;
	}
...
void writeByte(unsigned int address, char data) {
	if(address >= MSize) {
		cout << "ERROR: the address out of range in writeByte" << endl;
		return;
	}
...
uint32_t readWord(unsigned int address) {
	if(address >= MSize-WORDSIZE) {
		cout << "ERROR: the address out of range in readWord" << endl;
		return 0;
	}
...
void writeWord(unsigned int address, uint32_t data) {
	if(address >= MSize-WORDSIZE) {
		cout << "ERROR: the address out of range in writeWord" << endl;
		return;
	}
...
void progMem() {
	writeWord(0, (0xfffff << 12) | (2 << 7) | (LUI));  // 定义PC计数器的值从0开始依次执行 
	...
	writeWord(0, 0x0013ab73);
	writeWord(4, 0x0013db73);
	writeWord(8, 0x0013fb73);
	writeWord(12, 0x0000100f);
	writeWord(16, 0x00100073);
}


uint32_t PC, NextPC;    // 这些是送给cpu的数据 
uint32_t R[32];
uint32_t IR;
...
void decode(uint32_t instruction) {    // CPU的功能部分 
