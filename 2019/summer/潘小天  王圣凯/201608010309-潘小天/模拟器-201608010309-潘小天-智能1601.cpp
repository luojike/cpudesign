	...
	while(c != 'n') {
		...
		switch(opcode) {                                                                         //这个是在decode时的低7位的值，是操作码 
			...
			case LOAD://0x03 LOAD被编码为I类型 
				...
				break;
			case STORE://STORE指令 STORE被编码为S类型  
			...
			case CSRX://CSRX指令  Itype 						
				switch(funct3) {
					...
					case CSRRW://The CSRRW (Atomic Read/Write CSR) instruction atomically swaps values in the CSRs and integer registers
						...
						break;
					case CSRRS:
						/*CSRRS读取CSR中的值，0扩展，然后将其写入到整型寄存器rd，rs1的初始值被当做一个位掩码指定要在CSR中要设置的位位置，
						如果csr位可写，rs1中的任何高位都将导致在csr中设置相应的位。csr中的其他位不受影响（尽管csr在写入时可能会产生副作用）。*/ 
						{
